// Following arguments are not needed when AsciiDoc conversion is done by Gradle plug-in(s).
...
confluence.with {
    input = [
            [ file: "build/asciidoc/html5/example-manual.html", ancestorId: 99999999 ],
    ]
...
def user = System.getenv()['CONFLUENCE_CREDENTIALS_USR']
def pass = System.getenv()['CONFLUENCE_CREDENTIALS_PSW']

if (user && pass) {
    println "Confluence authentication set by system environment variables"
    confluence.credentials = (user + ':' + pass).bytes.encodeBase64().toString()
} else if (confluence.credentials) {
    println "Confluence authenticatie set by configuration file"
} else {
    throw new IllegalArgumentException("No Confluence credentials are set by environment variables or configuration file, see README.md")
}
